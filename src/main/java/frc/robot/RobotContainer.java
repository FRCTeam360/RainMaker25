...
public class RobotContainer {
  ...
  public void initializeCommands() {
    ...
    visionShootAlgae =
        new VisionShootAlgae(driveTrain, vision, algaeRoller, algaeShooter, algaeTilt, driverCont);
    ...
    if (Objects.nonNull(elevator)) {
      ...
      autoLevelThree =
          commandFactory.setElevatorHeight(SetPointConstants.ElevatorHeights.AUTO_LEVEL_THREE);
      autoLevelFour =
      ...
      registerPathplannerCommand(
          "zero", commandFactory.setElevatorHeight(0.0).raceWith(elevator.isAtHeight(0.0)));
    }

    if (Objects.nonNull(driveTrain)) {
      teleRightAlign =
          commandFactory.teleAlignWithLimelight(
      ...
      autoRightAlign =
          commandFactory.autoAlignWithLimelight(
    ...
    if (Objects.nonNull(coralShooter) && Objects.nonNull(elevator)) {
      ...
      removeAlgae =
          new RemoveAlgae(
              algaeArm, algaeShooter, algaeRoller, algaeTilt, coralShooter, elevator, vision);
    }
  ...
  /**
   * This method registers the given command to as a named pathplanner command if the command is
   * present. If not, it registers in its place a placeholder command that outputs a warning to the
   * console
   *
    ...
    driverCont
        ...
        .whileTrue(
            new InstantCommand(() -> toggleIsAlgaeMode())
                .andThen(
                    Commands.either(
                        Commands.none(), commandFactory.homeAlgaeTilt(), () -> isAlgaeMode))
                .alongWith(
                    Commands.either(
                        Commands.none(), elevator.setElevatorHeight(0.0), () -> !isAlgaeMode))
                .alongWith(
                    Commands.either(
                        new InstantCommand(
  ...
  private void configureTestController() {
    ...
    // testCont.x().whileTrue(elevator.setElevatorHeight(0.5));
    testCont.a().whileTrue(elevator.setElevatorHeight(0.5 * 2));
    testCont.b().whileTrue(elevator.setElevatorHeight(0.5 * 3));
    testCont.y().whileTrue(elevator.setElevatorHeight(0.5 * 4));
    testCont.x().whileTrue(elevator.setDutyCycleCommand(() -> 0.7));
