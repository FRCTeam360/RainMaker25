...
public class AlignWithLimelight extends Command {
  ...
  private NetworkTable table =
      NetworkTableInstance.getDefault().getTable(Constants.CompBotConstants.CORAL_LIMELIGHT_NAME);
  ...
  private void driveRobot() {
    double velX =
        -driveTrain.forwardController.calculate(
            table.getEntry("ty").getDouble(0.0), goalTY, driveTrain.getState().Timestamp);

    double velY =
        driveTrain.strafeController.calculate(
            table.getEntry("tx").getDouble(0.0), goalTX, driveTrain.getState().Timestamp);
    ...
    Translation2d PIDSpeed =
        new Translation2d(
            forwardsAccelerationLimit.calculate(velX), leftAccelerationLimit.calculate(velY));
