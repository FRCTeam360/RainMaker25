...
public class BargeAlign extends Command {
  ...
  private static final Map<Integer, Double> tagIDToAngle =
      Map.ofEntries( // blue pov
          Map.entry(15, -90.0), Map.entry(14, -90.0), Map.entry(4, 90.0), Map.entry(5, 90.0));
  ...
  public void initialize() {
    ...
    id =
        vision.getAprilTagID(
            Constants.isCompBot()
                ? CompBotConstants.ALGAE_LIMELIGHT_NAME
  ...
  @Override
  public void execute() {
    if (driveTrain.forwardController.atSetpoint()
        && driveTrain.headingController.atSetpoint()
        && algaeShooter.getVelocity() > 5850) {
  ...
  private void setPose() {
    ...
    double velX =
        driveTrain.forwardController.calculate(
    ...
    double velY =
        Math.pow(MathUtil.applyDeadband(-cont.getLeftX(), 0.1), 3) * driveTrain.getMaxSpeed();
