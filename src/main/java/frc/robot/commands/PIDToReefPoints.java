...
public class PIDToReefPoints {
  private static class ReefPositions {
    private static final Map<Integer, Translation2d> LEFT_TAG_ID_TO_POSITION_OFFSET =
        Map.ofEntries(
            Map.entry(6, new Translation2d(0, 0)),
            Map.entry(7, new Translation2d(0, 0)),
            Map.entry(8, new Translation2d(0, 0)),
            Map.entry(9, new Translation2d(0, 0)),
            Map.entry(10, new Translation2d(0, 0)),
            Map.entry(11, new Translation2d(0, 0)),
            Map.entry(17, new Translation2d(0, 0)),
            Map.entry(18, new Translation2d(0, 0)),
            Map.entry(19, new Translation2d(0, 0)),
            Map.entry(20, new Translation2d(0, 0)),
            Map.entry(21, new Translation2d(0, 0)),
            Map.entry(22, new Translation2d(0, 0)));
    // RIGHT STARTS HERE
    private static final Map<Integer, Translation2d> RIGHT_TAG_ID_TO_POSITION =
        Map.ofEntries(
            Map.entry(6, new Translation2d(0, 0)),
            Map.entry(7, new Translation2d(0, 0)),
            Map.entry(8, new Translation2d(0, 0)),
    ...
    private static final Set<Integer> REEF_TAG_IDS =
        Set.of(6, 7, 8, 9, 10, 11, 17, 18, 19, 20, 21, 22);

    private static final List<AprilTag> REEF_TAGS =
        Constants.FIELD_LAYOUT.getTags().stream()
            .filter(tag -> REEF_TAG_IDS.contains(tag.ID))
            .collect(
                Collectors.collectingAndThen(Collectors.toList(), Collections::unmodifiableList));
    private static final List<AprilTag> REEF_TAGS_RED =
        Constants.FIELD_LAYOUT.getTags().stream()
            .filter(tag -> REEF_TAG_IDS_RED.contains(tag.ID))
            .collect(
                Collectors.collectingAndThen(Collectors.toList(), Collections::unmodifiableList));
    private static final List<AprilTag> REEF_TAGS_BLUE =
        Constants.FIELD_LAYOUT.getTags().stream()
            .filter(tag -> REEF_TAG_IDS_BLUE.contains(tag.ID))
            .collect(
                Collectors.collectingAndThen(Collectors.toList(), Collections::unmodifiableList));

    private static final Map<Integer, Pose2d> LEFT_TAG_ID_TO_POSITION_DYNAMIC =
        REEF_TAGS.stream()
            .map(tag -> createTagScoringPositionEntry(tag, false))
            .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

    private static final Map<Integer, Pose2d> RIGHT_TAG_ID_TO_POSITION_DYNAMIC =
        REEF_TAGS.stream()
            .map(tag -> createTagScoringPositionEntry(tag, true))
            .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

    private static Entry<Integer, Pose2d> createTagScoringPositionEntry(
        AprilTag tag, boolean right) {
    ...
    private static Pose2d transformTagPoseToScoringPose(AprilTag tag, boolean right) {
      ...
      if (right) {
        ...
        Translation2d tagTranslation = new Translation2d(0.45, 0.10).plus(offset);
        ...
        // x is forwards-backwards, y is right-left relative to the tag position
        Translation2d tagTranslation = new Translation2d(0.45, -0.26).plus(offset);
        tagTranslation.rotateBy(tagRotation);
        return tagPose.plus(new Transform2d(tagTranslation, Rotation2d.k180deg));
      }
    }
  ...
  /**
   * WORK IN PROGRESS Moves to the closest reef scoring position on the robot's alliance side
   *
   * @param drivetrain
   * @param currentBotPose supplier for the robot's current position
   * @param right if true, move to the right pole of the reef. If false, move to the left pole
   * @return Command to path find to the reef
   */
  public static Command pidToReefOptimized(
      CommandSwerveDrivetrain drivetrain, Supplier<Pose2d> currentBotPose, boolean right) {
    initializeConstants();
    // Create commands
    return findNearestTagFirst(currentBotPose.get())
        .andThen(
            Commands.either(
                // Red Alliance
  ...
  private static Command findNearestTagFirst(Pose2d currentBotPose) {
    return new InstantCommand(
        () -> nearestTag = getNearestReefTagID(currentBotPose, isRedAlliance()));
  }
   ...
   * @return Command to path find to the reef
   */
  public static Command pidToReef(
      CommandSwerveDrivetrain drivetrain, Supplier<Pose2d> currentBotPose, boolean right) {
    initializeConstants();
    // Create commands
  ...
  private static Command moveToReefRightOrLeft(
      CommandSwerveDrivetrain drivetrain,
      Supplier<Pose2d> currentBotPose,
      boolean right,
      boolean isRed) {
    Map<Integer, Pose2d> tagIDPositionMapRight =
        logPositions(
            ReefPositions.RIGHT_TAG_ID_TO_POSITION_DYNAMIC, "RIGHT_TAG_ID_TO_POSITION_DYNAMIC");
    Map<Integer, Pose2d> tagIDPositionMapLeft =
        logPositions(
            ReefPositions.LEFT_TAG_ID_TO_POSITION_DYNAMIC, "LEFT_TAG_ID_TO_POSITION_DYNAMIC");
    return Commands.either(
  ...
  private static Map<Integer, Pose2d> logPositions(
      Map<Integer, Pose2d> tagIDPositionMap, String name) {
  ...
  /**
   * Helper method to fill out the select commands for the left and right sides of the reef
   *
   * @param drivetrain
   ...
   * @return
   */
  private static Command moveToReefHelper(
      CommandSwerveDrivetrain drivetrain,
      Supplier<Pose2d> currentBotPose,
      Map<Integer, Pose2d> tagIDPositionMap,
      boolean isRed) {
    return Commands.select(
        buildMoveToReefCommandMap(drivetrain, tagIDPositionMap),
        () -> getNearestReefTagID(currentBotPose.get(), isRed));
  }

  private static Map<Integer, Command> buildMoveToReefCommandMap(
      CommandSwerveDrivetrain drivetrain, Map<Integer, Pose2d> tagIDPositionMap) {
    return ReefPositions.REEF_TAG_IDS.stream()
        .map(tagId -> buildMoveToReefCommandEntry(drivetrain, tagId, tagIDPositionMap))
        .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
  }

  private static Entry<Integer, Command> buildMoveToReefCommandEntry(
      CommandSwerveDrivetrain drivetrain, int tagID, Map<Integer, Pose2d> tagIDPositionMap) {
  ...
  private static int getNearestTagID(Pose2d currentBotPose, List<AprilTag> tags) {
    ...
    for (AprilTag tag : tags) {
      double distance =
          currentBotPose.getTranslation().getDistance(tag.pose.toPose2d().getTranslation());
      if (distance < minDist) {
        closestTag = tag;
        minDist = distance;
      }
