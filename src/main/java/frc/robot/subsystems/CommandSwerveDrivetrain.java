...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
    ...
    public void periodic() {
        ...
        if (DriverStation.isDisabled() && DriverStation.isAutonomous()) {
            if(!rotationResetTimer.isRunning()){
                rotationResetTimer.restart();
            } else if (rotationResetTimer.hasElapsed(5.0)) {

                this.initializeRotationForAlliance();
                rotationResetTimer.restart();
            }
        } else if (rotationResetTimer.isRunning()) {
            rotationResetTimer.stop();
