...
public class CoralShooterIOPB implements CoralShooterIO {
  ...
  private final SparkMax outtakeMotor =
      new SparkMax(Constants.PracticeBotConstants.CORAL_SHOOTER_ID, MotorType.kBrushless);
  private final RelativeEncoder encoder = outtakeMotor.getEncoder();
  private final SparkMaxConfig sparkMaxConfig = new SparkMaxConfig();

  private final Canandcolor intakeSensor =
      new Canandcolor(Constants.PracticeBotConstants.INTAKE_SENSOR_ID);
  private final Canandcolor outtakeSensor =
      new Canandcolor(Constants.PracticeBotConstants.OUTTAKE_SENSOR_ID);
  ...
  private boolean isInOuttakeSensor() {
    return outtakeSensor.getProximity() < 0.2; // tuned for praccy bot 3/15
  }
