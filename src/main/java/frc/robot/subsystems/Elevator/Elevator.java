...
public class Elevator extends SubsystemBase {
  ...
  public Command setElevatorHeightDynamic(double height, DoubleSupplier tySupplier) {
    return Commands.waitUntil(() -> tySupplier.getAsDouble() < 0.0) // TODO: replace
        .andThen(
            Commands.waitUntil(
                    () -> {
                      return Math.abs(
                              inputs.elevatorPosition
                                  - getElevatorHeightWithOffset(height, tySupplier))
                          <= 0.5;
                    })
                .deadlineFor(
                    this.runEnd(
  ...
  public Command setElevatorHeight(double height) {
    return Commands.waitUntil(() -> Math.abs(inputs.elevatorPosition) <= 0.5)
        .deadlineFor(
            this.runEnd(() -> io.setElevatorPostion(height), () -> io.setElevatorPostion(height)));
  }
  ...
  public void periodic() {
    ...
    Logger.recordOutput("Elevator: periodic loop time", (periodicLoopTime / 1000.0));
  }
}
