...
public class Vision extends SubsystemBase {
    ...
    static {
        MEASUREMENT_STD_DEV_DISTANCE_MAP.put(0.5, VecBuilder.fill(1.0, 1.0, 999999.0)); // n1 and n2 are for x and y, n3
                                                                                        // is for angle
        MEASUREMENT_STD_DEV_DISTANCE_MAP.put(6.0, VecBuilder.fill(10.0, 10.0, 999999.0));
    }
    ...
    public Command waitUntilTargetTxTy(String name, double goalTX, double goalTY) {
        return Commands
                .waitUntil(() -> isTargetInView(name) && isOnTargetTX(name, goalTX) && isOnTargetTY(name, goalTY));
    }
    ...
    public void periodic() {
        ...
        for (String key : visionInputs.keySet()) {
            ...
            Matrix<N3, N1> cprStdDevs = MEASUREMENT_STD_DEV_DISTANCE_MAP
                    .get(closestTagDistance.orElse(Double.MAX_VALUE));
    ...
    public Command consumeVisionMeasurements(Consumer<List<VisionMeasurement>> visionMeasurementConsumer) {
        return CommandLogger.logCommand(run(() -> visionMeasurementConsumer.accept(acceptedMeasurements)),
